...
export function registerControllingTools(server: McpServer, m3: MoneyS3Client) {
  server.tool(
    ...
    async ({ take, skip }) => {
      ...
      if (!cc?.items?.length) return { content: [{ type: "text", text: "No cost centers found." }] };
  ...
  server.tool(
    ...
    async ({ take, skip }) => {
      ...
      if (!p?.items?.length) return { content: [{ type: "text", text: "No projects found." }] };
  ...
  server.tool(
    ...
    async ({ take, skip }) => {
      ...
      if (!a?.items?.length) return { content: [{ type: "text", text: "No activities found." }] };
  ...
  createCostCenter(
    ...
    definitionXMLTransfer: { shortCut: "${escGql(definitionShortcut)}" }
      ...
      return { content: [{ type: "text", text: `Cost center "${code}" ${result.isSuccess ? "created" : "queued"}.\nGUID: \`${result.guid}\`` }] };
    },
  );
  ...
  createProject(
    ...
    definitionXMLTransfer: { shortCut: "${escGql(definitionShortcut)}" }
      ...
      return { content: [{ type: "text", text: `Project "${code}" ${result.isSuccess ? "created" : "queued"}.\nGUID: \`${result.guid}\`` }] };
    },
  );
  ...
  createActivity(
    ...
    definitionXMLTransfer: { shortCut: "${escGql(definitionShortcut)}" }
      ...
      return { content: [{ type: "text", text: `Activity "${code}" ${result.isSuccess ? "created" : "queued"}.\nGUID: \`${result.guid}\`` }] };
    },
  );
}
